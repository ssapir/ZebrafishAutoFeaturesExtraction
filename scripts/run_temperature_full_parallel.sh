...
CONFIG_PATH=$dir/parameters_temperature.cfg
...
echo "Input params: Dataset_path $dataset_path, args $args, rerun $rerun, size $size, repository $repository_relative_to_script_path, opencv_conda_env $opencv_conda_env"
...
jobarrout=$(sbatch --job-name=$fish-full --array 0-$num $dir/slurm_files/run_jobarr_full_main.sh $dataset_path $fish $size $repository_relative_to_script_path $opencv_conda_env --fast_run)
