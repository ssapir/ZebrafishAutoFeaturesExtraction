...
class Head:
    ...
    def validate_eyes_data(eyes_abs_angle_list, eyes_head_dir_diff_ang_list):
        ...
        for curr_eyes_list, name_ in zip([eyes_abs_angle_list, eyes_head_dir_diff_ang_list],
            ...
            # if curr_eyes_list.shape == (2,):
            #     print("event has no frames")
...
class Tail:
    def __init__(self, tail_tip_points: Points, is_bout_frame_list: List[bool], tail_path_list: List[Points],
        ...
        self.tail_path_list = tail_path_list
    ...
    def export_to_struct(self):
        tail_path_list_exported = []
        for tail_path in self.tail_path_list:
            if type(tail_path) is Points:  # todo this is a bug
                tail_path = tail_path.export_to_struct()

            tail_path_list_exported.append(tail_path)
    ...
    @classmethod
    def import_from_struct(cls, data):
        if 'tail_path_list' not in data.keys() or len(data['tail_path_list']) == 0:
        ...
        return cls(tail_tip_points=Points.import_from_struct(data['tail_tip_point_list']),
                   is_bout_frame_list=data['is_bout_frame_list'],
                   tail_path_list=tail_path_list_as_points,
                   velocity_norms=data.get('velocity_norms', data.get('velocity_norms_convolution', [])))
...
class Paramecium:
    ...
    def center_points(self):
        ...
        :return: nd.array center: nXmX2 where 2 = the centroid of each paramecia at each frames is (x,y).
        """
    ...
    def import_from_struct(cls, data):  # match ctor
        return cls(center=data['center'], area=data['area'], status=data['status'], color=data['color'],
                   ellipse_majors=data['ellipse_majors'], ellipse_minors=data['ellipse_minors'],
                   ellipse_dirs=data['ellipse_dirs_in_deg'], bounding_boxes=data['bounding_boxes'])
...
class Event:
    ...
    def __init__(self, event_name: str, event_id: int, head: Head, tail: Tail, fish_tracking_status_list: List[bool],
                 is_head_prediction_list:List[bool], tail_tip_status_list: List[bool], paramecium: Paramecium):
    ...
    def from_tracker_output(cls,
        ...
        for tail_path in tail_path_list:
            if type(tail_path) is not Points:
                Points.from_array(tail_path, cast_to_float=True)
